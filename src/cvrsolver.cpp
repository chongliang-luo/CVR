...
// /// install.packages("RcppArmadillo_0.9.900.2.0.tar.gz", repos = NULL, type = "source")
// /// the  gradient / hessian diagonal of Cox PH log-partial likelihood function w.r.t. the linear predictor
// /// time is ordered!!!
// // [[Rcpp::export]]
// Rcpp::List rcpp_coxph_deriv_lp(arma::vec event, arma::vec os, double theta, arma::vec lp){  
//   arma::vec exp_lp = exp(os+lp*theta);
//   // std::cout << "chkpt-0 " << exp_lp;
//   arma::vec h0_denom = reverse(cumsum(reverse(exp_lp)));   
//   // std::cout << "chkpt-1 " << h0_denom ;
//   
//   arma::vec cs_elp_inv_sum_R_exp = cumsum(event/h0_denom)%exp_lp;
//   // std::cout << "chkpt-2 " << cs_elp_inv_sum_R_exp;
//   arma::vec cs_elp_inv_sum_R_exp2 = cumsum(event/h0_denom/h0_denom)%exp_lp%exp_lp;
//   // std::cout << "chkpt-3 " << cs_elp_inv_sum_R_exp2;
//   
//   Rcpp::List out;
//   out["gradient"] = (event - cs_elp_inv_sum_R_exp)*theta;          // gradient
//   out["hessianD"] = -(cs_elp_inv_sum_R_exp-cs_elp_inv_sum_R_exp2)*theta*theta;  // hessian diagonal
//   return(out);
// }
...
double coxph_lpl(arma::vec event, arma::vec lp){ 
  // arma::vec time, 
  // coxph neg log partial lik, no tie, data ordered by time
  arma::vec exp_lp = exp(lp); 
  arma::vec h0_denom = reverse(cumsum(reverse(exp_lp)));   
  return(- arma::sum(event%(lp - arma::log(h0_denom))));
}

// [[Rcpp::export]]
arma::mat Newtoncox(arma::vec GZ, arma::vec Bbeta, double mu, double h, arma::vec Zini,
                    arma::vec event){ // arma::vec time,  // vec or mat...
  int maxiter = 20;
  int n = GZ.n_rows;
  int q = GZ.n_cols;
  arma::vec Z = Zini;
  arma::vec M = - GZ/h + Bbeta;        // conv_to< vec >::from(B*beta)
  arma::vec expZ, dz, Hz, h0_denom, cs_elp_inv_sum_R_exp, cs_elp_inv_sum_R_exp2; 
  int iter = 0;
  for (iter = 0; iter < maxiter; iter++){ 
    expZ = exp(Z); 
    h0_denom = reverse(cumsum(reverse(expZ)));   
    cs_elp_inv_sum_R_exp = cumsum(event/h0_denom)%expZ;
    cs_elp_inv_sum_R_exp2 = cumsum(event/h0_denom/h0_denom)%expZ%expZ;
    
    dz = - mu*(event - cs_elp_inv_sum_R_exp)/n + h*(Z - M);
    Hz = mu*(cs_elp_inv_sum_R_exp-cs_elp_inv_sum_R_exp2)/n + h;
    Hz = Hz*ones(1, q);
    Z = Z - dz/Hz;           
  } 
...
//' @export
// [[Rcpp::export]]
Rcpp::List cvrsolver(arma::mat Y, Rcpp::List Xlist, arma::mat event, int rank, double eta, arma::vec Lam,  // arma::mat event, 
  ...
  if (family=="gaussian"){
    ...
    // cout << "Checknode 1: setups" << endl;
    for (int k = 0; k < K; k++){
      Z[k] = zeros(n);  // for now Z is a vector (univariate event time...)
      Z[k] = Newtoncox(GZ[k],B[k]*beta,1-eta,h,Z[k], event);   // B[k]*beta
    }
  } 
  ...
  for (iter = 0; iter < maxIters; iter++){
    ...
    for (int k = 0; k < K; k++){
      if(family=="cox"){
        C[k] = join_rows(zeros(n, 1), B[k]);   // no intercept for cox  
      }else{
    ...
    if (family=="binomial"){
      ...
      for (int k = 0; k < K; k++){
        Ztmp = XW[k]*beta;  // + ones(n, 1)*alpha
        tmp2 = tmp2 + coxph_lpl(event, Ztmp)/n;
      }
    } 
